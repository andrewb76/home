...
import TheCertificationsModal from "@/components/cv/TheCertificationsModal.vue";
import TheProjects from "@/components/cv/TheProjects.vue";
import TheProjectsModal from "@/components/cv/TheProjectsModal.vue";
...
export default defineComponent({
  ...
  setup() {
    const cv = useCVStore();
    return { cv: cv.iAm, projects: cv.projects };
  },
});
</script>
<template>
  <main class="md:container md:mx-auto main-container md:m-4">
    <div class="about grid gap-5 lg:grid-cols-3">
      <div class="space-y-5">
        ...
        />
        <the-educations :data="cv.education" />
        <the-languages :data="cv.languages" />
        <the-certifications :data="cv.certificates" />
        <the-skills :data="cv.skills" />
      </div>
      <div class="space-y-5 lg:col-span-2">
        <!-- Start Right Side -->
        <the-summary :data="cv.summary" />
        <the-experience :data="cv.experience" />
        <the-projects />
      </div>
    </div>
  </main>
  <the-certifications-modal :data="cv.certificates" />
